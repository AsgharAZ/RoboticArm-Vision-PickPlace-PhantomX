function pick_place(pick_coords, place_coords)
    % Initialize current state
    current_state = 0;
    
    % Offset in z-coordinates for pick and place locations
    pick_coords(3) = pick_coords(3) + 2;
    place_coords(3) = place_coords(3) + 2;
    
    % Height of the block (in cm)
    height_of_block = 2.8;
    ...
    while true
        ...
        if current_state == 0
            ...
            pause(5);
            error_ = error_estimator([0, 0, 0, 0, 0], [arb.getpos]);
            if error_ == false
                current_state = 1;
            else
                current_state = 0;
        ...
        elseif current_state == 2
            ...
            arb = Arbotix('port', 'COM18', 'nservos', 5);
            curr_pos = arb.getpos;
            error_1 = error_estimator([coords_1, 0], [curr_pos]);
            if error_1 == false
                current_state = 3;
            else
                current_state = 2;
        ...
        elseif current_state == 3
            ...
            arb = Arbotix('port', 'COM18', 'nservos', 5);
            curr_pos = arb.getpos();
            error_1 = error_estimator([coords_2, 0], [curr_pos]);
            if error_1 == false
                current_state = 4;
            else
                current_state = 3;
        ...
        elseif current_state == 6
            ...
            error_3 = error_estimator([coords_3, 1.1556], [curr_pos_]);
            if error_3 == false
                current_state = 7;
            else
                current_state = 6;
            ...
            arb = Arbotix('port', 'COM18', 'nservos', 5);
            curr_pos = arb.getpos();
            error_4 = error_estimator([coords_4_, 1.2], [curr_pos]);
            if error_4 == false
                current_state = 8;
            else
                current_state = 7;
